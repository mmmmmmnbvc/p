...
$adminStmt = $pdo->prepare("SELECT * FROM ADMIN WHERE USA = ? AND PSA = ?");
...
if (!empty($adminRow)) {
    $_SESSION["fullname"] = $adminRow["NameA"];
    $_SESSION["username"] = $adminRow["USA"];

    $_SESSION["user_id"] = $adminRow["id"]; 
    echo "เข้าสู่ระบบสำเร็จ (ADMIN)<br>";
    header("location: Userm.php?username=" . $username);
} elseif (!empty($memberRow)) {
