...
table {
    margin: 0 auto;
        ...
        function update(id) {
            ...
            document.location = "cart.php?action=update&id=" + id + "&qty=" + qty;
        }
    </script>
  </head>
...
<nav>
    <h3>
      ...
      <section>
        ...
        <article class="h" style="text-align: center;">
    <form action="purchase.php" method="POST">
        <table border="1">
            ...
            <tr>
                ...
                <td><?=$item["price"]?></td>
                <td>
                    <input type="number" name="qty[<?=$item["id"]?>]" id="<?=$item["id"]?>" value="<?=$item["qty"]?>" min="1" max="9">
                    <a href="#" onclick="update(<?=$item["id"]?>)">แก้ไข</a>
                    <a href="?action=delete&id=<?=$item["id"]?>">ลบ</a>
                </td>
            </tr>
            <?php
                }
            ...
            ?>
            <tr>
                <td colspan="4" align="right">รวม <?=$sum?> บาท</td>
            </tr>
        </table>

        <!-- เพิ่มส่วนสำหรับกรอกรหัสโปรโมชั่น -->
        <label for="promo_code">รหัสโปรโมชั่น:</label>
        <input type="text" name="promo_code" id="promo_code">
        <input type="submit" name="purchase" value="ซื้อ">
    </form>
