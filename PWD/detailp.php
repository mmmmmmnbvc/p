...
<nav>
   ...
   $stmt = $pdo->prepare("SELECT * FROM Menu_PCK WHERE id = ?");
...
<div>
     <img src='img/<?=$row["id"]?>.jpg' width='200'>
</div>
<div style="padding: 15px">
    <h2><?=$row["name_food"]?></h2>
    ราคาขาย <?=$row["price"]?> บาท<br>
    แคลอรี่ <?=$row["calorie"]?> บาท<br>
	<form method="post" action="cart.php?action=add&id=<?=$row["id"]?>&name_food=<?=$row["name_food"]?>&price=<?=$row["price"]?>
    &calorie=<?=$row["calorie"]?> ">
